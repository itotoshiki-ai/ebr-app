...
**「サーバーのホスト名」とは、コンピューター名のことです。** サーバー機のPowerShellで
`hostname`または`$env:COMPUTERNAME`を実行すると確認できます(「システムのプロパティ」の
「コンピューター名」と同じ値です)。IPアドレスは`ipconfig`の「IPv4 アドレス」で確認できます。

ホスト名とIPアドレスの**両方**を指定しているのは、この2つが必ずしも同じように使えるとは
限らないためです。IPアドレスは同一ネットワーク内であれば基本的にどの端末からも到達できますが、
ホスト名は社内ネットワークの名前解決の仕組み(Windows同士ならNetBIOS名解決、mDNS対応があれば
`ebr-server.local`等)に依存するため、環境によっては一部の端末(特にスマホ等)から名前だけでは
繋がらないことがあります。両方を証明書のSAN(Subject Alternative Name)に含めておくことで、
名前解決がうまくいかない端末はIPアドレスでアクセスする、という運用にできます
(次の手順の`REDIRECT_URI`・Azure Portal側のリダイレクトURI登録も、実際に使う方
(ホスト名 or IP)に合わせて選んでください)。
...
## デプロイ後にプログラムを修正した場合の反映手順

常時稼働サーバーへのデプロイ後にコードを修正した場合は、以下の手順で反映します。
開発機側でコミット・pushまで終えていることが前提です。以降はすべて**サーバー機**の
PowerShellで、リポジトリのディレクトリ内(`C:\apps\ebr-app`)で実行します。
...
`package.json`/`package-lock.json`が変更に含まれる場合のみ実行(変更なしならスキップ可)。
...
`.env.example`に新しい変数が追加されていても、`.env`自体は`git pull`で自動更新されません
(gitignore対象のため)。差分を見て、必要な値を手動で`.env`に追記してください。
...
```
nssm restart ebr-app
```

(`restart`が効かない場合は`nssm stop ebr-app` → `nssm start ebr-app`)

コンソールで直接`npm start`して動かしている場合は、そのウィンドウで`Ctrl+C`で停止してから
`npm start`し直します。

### 動作確認

サーバー機、または社内の他端末のブラウザで`https://<サーバー>:3000`を開き、正常に動作するか
確認します。

### 注意点

- サービス再起動時、**その時点でアップロード済みだが未保存(「OneDriveに保存」を押す前)の
  抽出結果は失われます**(アップロード直後の抽出結果はメモリ上のみで一時保持する仕組みのため、
  サーバー再起動でリセットされます)。他の利用者が作業中でないか確認してから再起動するのが
  安全です。
- サインイン状態・保存先フォルダ設定(`data/sessions.json`)や証明書(`certs/`)はgitignore対象
  なので、`git pull`によって消えたり上書きされたりすることはありません。
